...
Account identity, baseline records, track metadata, matches, queue state, retries, and completed paths are kept in a local SQLite database. Track names, artists, albums, duration, or recording identifiers may be sent to the configured Tidal mirror service to find a matching recording. Google/YouTube credentials are never sent to the audio provider. The published endpoint configuration is fetched from GitHub; reusable service access is encrypted with current-user DPAPI. No browser is used for audio downloads. Legacy browser data may remain locally after an upgrade, but is no longer used. Audio and artwork requests reach their hosting providers.
